...
void _mutt_make_string(char *, size_t, const char *, CONTEXT *, HEADER *, format_flag);
...
void mutt_make_string_info(char *, size_t, int, const char *,
                           struct hdr_format_info *, format_flag);
...
#define mutt_collapse_thread(x, y)                                             \
  _mutt_traverse_thread(x, y, MUTT_THREAD_COLLAPSE)
#define mutt_uncollapse_thread(x, y)                                           \
  _mutt_traverse_thread(x, y, MUTT_THREAD_UNCOLLAPSE)
#define mutt_get_hidden(x, y)                                                  \
  _mutt_traverse_thread(x, y, MUTT_THREAD_GET_HIDDEN)
#define mutt_thread_contains_unread(x, y)                                      \
  _mutt_traverse_thread(x, y, MUTT_THREAD_UNREAD)
...
void mutt_FormatString(char *, size_t, size_t, int, const char *, format_t *,
                       unsigned long, format_flag);
...
#define mutt_mktemp_pfx_sfx(a, b, c, d)                                        \
  _mutt_mktemp(a, b, c, d, __FILE__, __LINE__)
...
#define mutt_enter_fname(A, B, C, D, E)                                        \
  _mutt_enter_fname(A, B, C, D, E, 0, NULL, NULL, 0)
#define mutt_enter_vfolder(A, B, C, D, E)                                      \
  _mutt_enter_fname(A, B, C, D, E, 0, NULL, NULL, MUTT_SEL_VFOLDER)
...
#ifdef LOCALES_HACK
#define IsPrint(c)                                                             \
  (isprint((unsigned char) (c)) || ((unsigned char) (c) >= 0xa0))
...
int mutt_pattern_exec(struct pattern_t *pat, pattern_exec_flag flags,
                      CONTEXT *ctx, HEADER *h, pattern_cache_t *);
