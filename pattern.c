...
static int is_context_available(BUFFER *s, regmatch_t pmatch[], int kind, BUFFER *err)
{
  char *context_loc;
...
static int perform_and(pattern_t *pat, pattern_exec_flag flags, CONTEXT *ctx,
...
void mutt_check_simple(char *s, size_t len, const char *simple)
{
  ...
  if (do_simple) /* yup, so spoof a real request */
  {
    ...
    if (ascii_strcasecmp("all", s) == 0 || !mutt_strcmp("^", s) ||
        !mutt_strcmp(".", s)) /* ~A is more efficient */
      strfcpy(s, "~A", len);
