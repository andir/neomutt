...
static void init_state(struct browser_state *state, MUTTMENU *menu)
{
  ...
  state->entrymax = 256;
  state->entry =
      (struct folder_file *) safe_calloc(state->entrymax, sizeof(struct folder_file));
#ifdef USE_IMAP
  state->imap_browse = 0;
#endif
  if (menu)
                             ...
                             char *d, const char *prefix)
{
  ...
  if (option(OPTNEWS))
  {
    ...
    for (i = 0; i < nserv->groups_num; i++)
    {
      ...
      if (!nntp_data)
        continue;
      if (prefix && *prefix && strncmp(prefix, nntp_data->group, strlen(prefix)))
        continue;
...
#endif /* USE_NNTP */
  {
    ...
    while ((de = readdir(dp)) != NULL)
    {
      ...
      while (tmp && mutt_strcmp(buffer, tmp->path))
        tmp = tmp->next;
      if (tmp && Context && !mutt_strcmp(tmp->realpath, Context->realpath))
      {
...
static int examine_mailboxes(MUTTMENU *menu, struct browser_state *state)
{
...
#endif
  {
    ...
    do
    {
      if (Context && !mutt_strcmp(tmp->realpath, Context->realpath))
      {
        tmp->msg_count  = Context->msgcount;
        tmp->msg_unread = Context->unread;
      }
