...
#define SENDPOSTPONEDFCC                                                       \
  (1 << 9)                         /* used by mutt_get_postponed() to signal that the x-mutt-fcc header field was present */
...
typedef enum { MUTT_MATCH_FULL_ADDRESS = 1 } pattern_exec_flag;
...
#define MUTT_PENDINGPREFIX                                                     \
  (1 << 2)                      /* prefix to write, but character must follow */
