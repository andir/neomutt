...
static void pseudo_threads(CONTEXT *ctx)
{
  ...
  while (tree)
  {
	 ...
	 * we're attaching. */
        if (tmp == cur ||
            !mutt_strcmp(tmp->message->env->real_subj, parent->message->env->real_subj))
        {
...
static void check_subjects(CONTEXT *ctx, int init)
{
  ...
  for (i = 0; i < ctx->msgcount; i++)
  {
    ...
    if (!tmp)
      ...
      cur->subject_changed =
          mutt_strcmp(cur->env->real_subj, tmp->message->env->real_subj) ? 1 : 0;
    else
      cur->subject_changed =
          (cur->env->real_subj || tmp->message->env->real_subj) ? 1 : 0;
  }
}
    ...
    if (!cur->thread)
    {
      ...
      else
      {
        ...
        hash_insert(ctx->thread_hash,
                    cur->env->message_id ? cur->env->message_id : "", thread, 1);
