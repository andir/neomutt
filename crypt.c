...
int mutt_protect(HEADER *msg, char *keylist)
{
  ...
  {
    if ((msg->content->type != TYPETEXT) || ascii_strcasecmp(msg->content->subtype, "plain"))
    {
...
int mutt_is_multipart_signed(BODY *b)
{
  ...
  if (!b || !(b->type == TYPEMULTIPART) || !b->subtype || ascii_strcasecmp(b->subtype, "signed"))
    return 0;
...
int mutt_is_multipart_encrypted(BODY *b)
{
  if ((WithCrypto & APPLICATION_PGP))
  {
    ...
    if (!b || b->type != TYPEMULTIPART || !b->subtype ||
        ascii_strcasecmp(b->subtype, "encrypted") ||
...
int mutt_is_valid_multipart_pgp_encrypted(BODY *b)
{
  ...
  if (!b || b->type != TYPEAPPLICATION || !b->subtype ||
      ascii_strcasecmp(b->subtype, "pgp-encrypted"))
    return 0;

  b = b->next;
  if (!b || b->type != TYPEAPPLICATION || !b->subtype ||
      ascii_strcasecmp(b->subtype, "octet-stream"))
    return 0;
...
int mutt_is_malformed_multipart_pgp_encrypted(BODY *b)
{
  ...
  if (!b || b->type != TYPEMULTIPART || !b->subtype || ascii_strcasecmp(b->subtype, "mixed"))
    return 0;

  b = b->parts;
  if (!b || b->type != TYPETEXT || !b->subtype ||
      ascii_strcasecmp(b->subtype, "plain") || b->length != 0)
    return 0;
...
int mutt_is_application_pgp(BODY *m)
{
  ...
  if (m->type == TYPEAPPLICATION)
  {
    if (!ascii_strcasecmp(m->subtype, "pgp") || !ascii_strcasecmp(m->subtype, "x-pgp-message"))
    {
      ...
      if ((p = mutt_get_parameter("format", m->parameter)) && !ascii_strcasecmp(p, "keys-only"))
        t |= PGPKEY;
...
int mutt_signed_handler(BODY *a, STATE *s)
{
  ...
  else
  {
    ...
    {
      case SIGN:
        if (a->next->type != TYPEMULTIPART || ascii_strcasecmp(a->next->subtype, "mixed"))
          inconsistent = 1;
        break;
      case PGPSIGN:
