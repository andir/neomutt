...
/*
  ...
  */
    {"attach_keyword", DT_RX, R_NONE, UL &AttachKeyword,
     UL "\\<(attach|attached|attachments?)\\>"},
    /*
  ** .pp
...
/*
  ...
  */
    {"compose_format", DT_STR, R_BOTH, UL &ComposeFormat,
     UL "-- NeoMutt: Compose  [Approx. msg size: %l   Atts: %a]%>-"},
    /*
  ** .pp
...
/*
  ...
  */
   ...
   */
    ...
    {"nm_hidden_tags", DT_STR, R_NONE, UL &NotmuchHiddenTags,
     UL "unread,draft,flagged,passed,replied,attachment,signed,encrypted"},
    /*
   ** .pp
   ...
   */
    {"nm_query_window_current_search", DT_STR, R_NONE,
     UL &NotmuchQueryWindowCurrentSearch, UL ""},
    /*
   ** .pp
   ** This variable sets the time duration of a windowed notmuch query.
   ** Accepted values all non negative integers. A value of 0 disables the feature.
   */
    {"nm_query_window_current_position", DT_NUM, R_NONE,
     UL &NotmuchQueryWindowCurrentPosition, 0},
/*
   ** .pp
...
/*
  ...
  */
    {"query_format", DT_STR, R_NONE, UL &QueryFormat,
     UL "%4c %t %-25.25a %-25.25n %?e?(%e)?"},
    /*
  ** .pp
...
/*
  ...
  ** "%B%?F? [%F]?%* %?N?%N/?%S".
  */
    {"sidebar_indent_string", DT_STR, R_SIDEBAR, UL &SidebarIndentString,
     UL "  "},
    /*
  ** .pp
...
/*
  ...
  */
    {"status_format", DT_STR, R_BOTH, UL &Status,
     UL "-%r-NeoMutt: %f [Msgs:%?M?%M/?%m%?n? New:%n?%?o? Old:%o?%?d? "
  ...
  */
    {"ts_status_format", DT_STR, R_BOTH, UL &TSStatusFormat,
     UL "NeoMutt with %?m?%m messages&no messages?%?n? [%n NEW]?"},
/*
  ** .pp
...
/*
  ...
  */
#ifdef USE_NOTMUCH
    {"vfolder_format", DT_STR, R_INDEX, UL &VirtFolderFormat,
     UL "%2C %?n?%4n/&     ?%4m %f"},
    /*
  ** .pp
...
const struct mapping_t SortMethods[] = {{"date", SORT_DATE},
                                        {"date-sent", SORT_DATE},
...
const struct mapping_t SortAuxMethods[] = {{"date", SORT_DATE},
                                           {"date-sent", SORT_DATE},
                                           {"date-received", SORT_RECEIVED},
                                           {"mailbox-order", SORT_ORDER},
                                           {"subject", SORT_SUBJECT},
...
const struct mapping_t SortKeyMethods[] = {{"address", SORT_ADDRESS},
                                           {"date", SORT_DATE},
                                           {"keyid", SORT_KEYID},
                                           {"trust", SORT_TRUST},
                                           {NULL, 0}};
...
const struct mapping_t SortSidebarMethods[] = {
    {"alpha", SORT_PATH},          {"count", SORT_COUNT},
    {"desc", SORT_DESC},           {"flagged", SORT_FLAGGED},
    {"mailbox-order", SORT_ORDER}, {"name", SORT_PATH},
    {"new", SORT_UNREAD}, /* kept for compatibility */
    {"path", SORT_PATH},           {"unread", SORT_UNREAD},
    {"unsorted", SORT_ORDER},      {NULL, 0}};
