...
#define MUTT_NEWFOLDER                                                         \
  (1 << 4) /* create a new folder - same as MUTT_APPEND, but uses
                                * safe_fopen() with mode "w" for mbox-style folders.
                                * This will truncate an existing file. */
#define MUTT_PEEK                                                              \
  (1 << 5) /* revert atime back after taking a look (if applicable) */
#define MUTT_APPENDNEW                                                         \
  (1 << 6) /* set in mx_open_mailbox_append if the mailbox doesn't
