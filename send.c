  ...
  else if (env->reply_to)
  {
    if ((mutt_addrcmp(env->from, env->reply_to) && !env->reply_to->next &&
        ...
        (option(OPTIGNORELISTREPLYTO) && mutt_is_mail_list(env->reply_to) &&
         (mutt_addrsrc(env->reply_to, env->to) || mutt_addrsrc(env->reply_to, env->cc))))
    {
    ...
    else if (!(mutt_addrcmp(env->from, env->reply_to) && !env->reply_to->next) &&
...
static void mutt_make_reference_headers(ENVELOPE *curenv, ENVELOPE *env, CONTEXT *ctx)
{
  env->references  = NULL;
                         ...
                         HEADER *cur)  /* current message */
{
  ...
  else if ((WithCrypto & APPLICATION_PGP) && (flags & SENDKEY))
  {
    ...
    if ((WithCrypto & APPLICATION_PGP) && (tmp = crypt_pgp_make_key_attachment(NULL)) == NULL)
      return -1;
...
int mutt_search_attach_keyword(char *filename)
{
                    ...
                    HEADER *cur)    /* current message */
{
  ...
  else if (!(flags & SENDBATCH))
  {
         ...
         query_quadoption(OPT_FORWEDIT, _("Edit forwarded message?")) == MUTT_YES))
    {
       ...
       */
      if (option(OPTTEXTFLOWED) && msg->content->type == TYPETEXT &&
          !ascii_strcasecmp("plain", msg->content->subtype))
      {
