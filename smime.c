...
void _smime_getkeys(char *mailbox)
{
  ...
  if (key)
  {
    ...
    if (*SmimeKeyToUse && !mutt_strcasecmp(k, SmimeKeyToUse + mutt_strlen(SmimeKeys) + 1))
    {
      smime_free_key(&key);
      return;
    }
  ...
  if (*SmimeKeyToUse)
  {
    if (!mutt_strcasecmp(SmimeDefaultKey, SmimeKeyToUse + mutt_strlen(SmimeKeys) + 1))
      return;
...
BODY *smime_sign_message(BODY *a)
{
  ...
  if ((!default_key) || (!mutt_strcmp("?", default_key->issuer)))
