...
#define DOUBLE_FAIL_STACK(fail_stack)                                          \
  ((fail_stack).size > (unsigned) (re_max_failures * MAX_FAILURE_ITEMS) ?      \
       0 :                                                                     \
       ((fail_stack).stack = REGEX_REALLOCATE_STACK(                           \
...
static reg_errcode_t compile_range _RE_ARGS((const char **p_ptr, const char *pend,
...
static boolean group_in_compile_stack _RE_ARGS((compile_stack_type compile_stack,
                                                regnum_t regnum));
      ...
      case '[':
      {
        ...
        for (;;)
        {
          ...
          { /* Leave room for the null.  */
            ...
            {
              PATFETCH(c);
              if (c == ':' || c == ']' || p == pend || (unsigned int) c1 == CHAR_CLASS_MAX_LENGTH)
                break;
              str[c1++] = c;
            }
            str[c1] = '\0';
...
static reg_errcode_t compile_range(p_ptr, pend, translate, syntax, b) const char **p_ptr,
    *pend;
...
static boolean group_match_null_string_p _RE_ARGS((unsigned char **p, unsigned char *end,
