...
#define xtoi_1(p)                                                              \
  (*(p) <= '9' ? (*(p) - '0') : *(p) <= 'F' ? (*(p) - 'A' + 10) : (*(p) - 'a' + 10))
#define xtoi_2(p) ((xtoi_1(p) * 16) + xtoi_1((p) + 1))

#define PKA_NOTATION_NAME "pka-address@gnupg.org"
#define is_pka_notation(notation)                                              \
  ((notation)->name && !strcmp((notation)->name, PKA_NOTATION_NAME))
...
static int crypt_id_matches_addr(ADDRESS *addr, ADDRESS *u_addr, crypt_key_t *key)
{
  ...
  if (addr->mailbox && u_addr->mailbox && mutt_strcasecmp(addr->mailbox, u_addr->mailbox) == 0)
    rv |= CRYPT_KV_ADDR;
                            ...
                            int idx, STATE *s, gpgme_signature_t sig)
{
  ...
  if ((sum & GPGME_SIGSUM_SIG_EXPIRED))
  {
    ...
    for (sig = result->signatures, i = 0; sig && (i < idx); sig = sig->next, i++)
      ;
...
static int verify_one(BODY *sigbdy, STATE *s, const char *tempfile, int is_smime)
{
  ...
  if (!badsig)
  {
    ...
    if (result)
    {
      for (signature = result->signatures; signature; signature = signature->next)
      {
        non_pka_notations = 0;
        for (notation = signature->notations; notation; notation = notation->next)
...
static const unsigned char *parse_dn_part(struct dn_array_s *array, const unsigned char *string)
{
                                       ...
                                       int *forced_valid, int oppenc_mode)
{
  ...
  *forced_valid = 0;
  ...
  mutt_debug(5, "crypt_getkeybyaddr: looking for %s <%s>.", a->personal, a->mailbox);
  ...
  *forced_valid = 0;
  ...
  for (k = keys; k; k = k->next)
  {
    ...
    if (!*p || (pfcopy && mutt_strcasecmp(pfcopy, crypt_fpr(k)) == 0) ||
        ...
        (ps && mutt_strcasecmp(ps, crypt_short_keyid(k)) == 0) || mutt_stristr(k->uid, p))
    {
      crypt_key_t *tmp;
    ...
    if (signature_key)
    {
      ...
      for (uid = key->uids; uid && ret; uid = uid->next)
      {
	     ...
	     */
            ...
            mailbox_match = (!strncmp(tmp_email, tmp_sender, mailbox_length));
            tmp_email += mailbox_length;
            tmp_sender += mailbox_length;
            domainname_match = (!strncasecmp(tmp_email, tmp_sender, domainname_length));
