...
static int check_certificate_cache(X509 *peercert)
{
  ...
  if (!X509_digest(peercert, EVP_sha1(), peermd, &peermdlen) || !SslSessionCerts)
  {
    return 0;
  }
...
static int ssl_verify_callback(int preverify_ok, X509_STORE_CTX *ctx)
{
  ...
  mutt_debug(
      1, "ssl_verify_callback: checking cert chain entry %s (preverify: %d)\n",
      X509_NAME_oneline(X509_get_subject_name(cert), buf, sizeof(buf)), preverify_ok);
...
static int interactive_check_cert(X509 *cert, int idx, int len)
{
  ...
  menu->title = title;
  if (SslCertFile && (option(OPTSSLVERIFYDATES) == MUTT_NO ||
