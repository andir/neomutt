...
static int mutt_check_boundary(const char *boundary, BODY *b)
{
  ...
  if ((p = mutt_get_parameter("boundary", b->parameter)) && !ascii_strcmp(p, boundary))
    return 1;
  return 0;
}
...
BODY *mutt_make_multipart(BODY *b)
{
  ...
  do
  {
    ...
    if (mutt_check_boundary(mutt_get_parameter("boundary", new->parameter), b))
...
ADDRESS *mutt_remove_duplicates(ADDRESS *addr)
{
  ...
  while (addr)
  {
    ...
    {
      if (tmp->mailbox && addr->mailbox && !ascii_strcasecmp(addr->mailbox, tmp->mailbox))
      {
        dup = 1;
        break;
      }
